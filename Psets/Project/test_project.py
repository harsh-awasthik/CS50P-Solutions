...
def test_convert_echelonform():
    ...
    assert np.allclose(p.convert_echelonForm(mat["large_matrix"], 9), mat["large_matrix_echelon"])
...
def test_convert_reducedechelon():
    ...
    assert np.allclose(p.convert_reducedechelon(mat["large_matrix_echelon"]), mat["large_matrix_reduced_echelon"])
    assert np.allclose(p.convert_reducedechelon(p.convert_echelonForm(mat['mat1'], 6)), mat['mat1_reducedechelon'])
